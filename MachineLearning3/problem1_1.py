"""
Clasification Algorithm Perceptron
...
class perceptron(object):
    ...
    def update_weights(self, inputs, label):
        self.weights = self.weights + self.learning_rate * self.error * inputs
        


class network(object):
    def __init__(self, size_input=2, size_output=1, error = 0.001):
        ## create the network
    ...
    def fit(self, iterations, training_data, labels):
        ...
        while error > self.min_error:
            for train_data, label in zip(training_data, labels):
                error, output = self.neuron1.neuron(train_data, label)
...
def main():
    ...
    weights, final_weights = Network.fit(1000000, datax, labels)
